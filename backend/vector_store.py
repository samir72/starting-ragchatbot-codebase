...
class VectorStore:
    ...
    def get_course_outline(self, course_name: str) -> Optional[Dict[str, Any]]:
        """
        Get course outline including title, link, and complete lesson list.

        Args:
            course_name: Course name or partial name (uses semantic search)

        Returns:
            Dictionary with 'title', 'course_link', and 'lessons' list, or None if not found
        """
        import json
        try:
